...
#pragma omp taskwait
    ...
    for(int ind=0; ind<3; ind++){
      Ci->M[ind].resize(NTERM, 0.0);
      Ci->L[ind].resize(NTERM, 0.0);
    }
...
#pragma omp single nowait
  ...
  void horizontalPass(Cell * Ci, Cell * Cj) {
    real_t r2 = norm(Ci->X - Cj->X);
    real_t R2 = r2 * THETA * THETA;
    real_t aux4 = 2*abs(sqrt(r2)-(Ci->R + Cj->R))/(Ci->sigma + Cj->sigma);
    // Here I introduce the regularizing neighborhood criterion with R3
    if     (R2 > (Ci->R + Cj->R) * (Ci->R + Cj->R) &&  aux4 > 1/PHI) {
        M2L(Ci, Cj);
